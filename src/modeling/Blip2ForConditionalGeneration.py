...
from src.modeling.Blip2PreTrainedModel import Blip2PreTrainedModel
...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel):
    ...
    def forward(
        ...
        image_embeds = self.vision_projection(pixel_values)
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        #    interpolate_pos_encoding=interpolate_pos_encoding,
        #).last_hidden_state
        vision_projection = nn.Linear(in_features=1536, out_features=1408).to(pixel_values.device)
        image_embeds = vision_projection(pixel_values)
        image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device)
        ...
        if not self.language_model.config.is_encoder_decoder:
            generate_kwargs["max_length"] = generate_kwargs.get("max_length", 30) + language_model_inputs.shape[1] - 1
